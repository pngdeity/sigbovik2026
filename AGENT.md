...
a linear program over measures. Solution is a finite sum of Dirac deltas —
$\sim$11 rings at specific $(r_j^*, z_j^*)$ with optimized mass weights.
Convex, likely unique, tractable via discrete LP on a grid. See `code/LP.md`.
...
The field constraint is far underdetermined: SVD analysis of the linearized
forward operator at the optimum shows only $\sim$11 well-determined modes out
of $n_{\rm src} \sim 5000$. Many $\rho$ distributions produce identical
fields to within $\varepsilon$. **The shape is determined by the optimality
criterion, not the physics.**
...
- **LP / ring solution** (`LP.md`): drop the $b(r)$ assumption; solve LP
  over $\rho(r,z)\geq 0$. Expected $\sim$11 sparse rings, potentially unique.
